...
public class MoveReleaseOffSlavesToMostFreeSlaves extends ArchiveType {
	...
	public MoveReleaseOffSlavesToMostFreeSlaves(Archive archive, SectionInterface section, Properties props, int confnum) {
		...
		if (_minnumOfSlaves != 0) {
			if (_minnumOfSlaves < 1) {
			    throw new IllegalArgumentException("minnumOfSlaves has to be > 0 for conf number " + confnum);
			}
		}
	}
    ...
    protected boolean isArchivedDir(DirectoryHandle lrf) throws IncompleteDirectoryException, OfflineSlaveException, FileNotFoundException {
    	...
    	return isArchivedToSpecificSlaves(lrf, _numOfSlaves,findDestinationSlaves(), _minnumOfSlaves);  	
    ...
    public String toString() {
    	return "MoveReleaseOffSlavesToMostFreeSlaves=[directory=[" + getDirectory().getPath() + "]dest=[" + outputSlaves(findDestinationSlaves()) + "]numOfSlaves=[" + _numOfSlaves + "]MinimumnumOfSlaves=[" + _minnumOfSlaves + "]]";
    }	
