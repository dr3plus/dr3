...
/**
 * @author CyBeR
 * @version $Id: ArchiveType.java 1945 2009-07-25 18:32:01Z djb61 $
 */
public abstract class ArchiveType {
	...
	/**
	 * Sets _slaveList, _numOfSlaves, _minnumOfSlaves, _section, _parent, and _archiveAfter Each
	...
	protected ArrayList<Job> recursiveSend(DirectoryHandle lrf) throws FileNotFoundException {
		...
		for (Iterator<FileHandle> iter = lrf.getFilesUnchecked().iterator(); iter.hasNext();) {
			...
			Job job = new Job(file, _priority, _numOfSlaves, findDestinationSlaves(), _minnumOfSlaves);
			jobQueue.add(job);
		}
	 ...
	 */
	protected static final boolean isArchivedToSpecificSlaves(DirectoryHandle lrf,int x,Set<RemoteSlave> rslaves, int minofSlaves) throws OfflineSlaveException {
		Set<DirectoryHandle> directories = null;
		Set<FileHandle> files = null;
		try {
			directories = lrf.getDirectoriesUnchecked();
		...
		for (Iterator<DirectoryHandle> iter = directories.iterator(); iter.hasNext();) {
			if (!isArchivedToSpecificSlaves(iter.next(), x, rslaves, minofSlaves)) {
				return false;
			}
		}
	...
	private void setProperties(Properties properties) {
		...
		_minnumOfSlaves = Integer.parseInt(properties.getProperty(_confnum + ".minnumofslaves", "0").trim());
