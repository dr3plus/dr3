 ...
 * @author zubov
 * @author mog
 * @version $Id: BasicHandler.java 2041 2010-04-11 18:36:43Z djb61 $
 */
public class BasicHandler extends AbstractHandler {
	...
	public AsyncResponse handleDeleteArchivejob(AsyncCommandArgument ac) {
		try {
			try {
				getSlaveObject().deleteArchivejob(mapPathToRenameQueue(ac.getArgs()));
			} catch (PermissionDeniedException e) {
				if (Slave.isWin32) {
					synchronized (getSlaveObject().getRenameQueue()) {
						getSlaveObject().getRenameQueue()
	...
	public AsyncResponse handleRemerge(AsyncCommandArgument ac) {
		try {
			...
			// boolean CRCRemerge = Boolean.parseBoolean(argsArray[5]) && !getSlaveObject().ignoreCRCRemerge();
			if (partialRemerge) {
		...
		/*	if (CRCRemerge) {
				logger.info("CRC mode enabled, sending CRC info if requested by master");
				sendResponse(new AsyncResponseSiteBotMessage("CRC remerge enabled"));
			} else {
				logger.info("CRC mode disabled, only running size check if requested by master");
				sendResponse(new AsyncResponseSiteBotMessage("CRC remerge disabled"));				
			}
		 */
	...
	public AsyncResponse handleChecksum(AsyncCommandArgument ac) {
		try {
			//TODO resolve remergeCheckSum from master <-> slave
			//boolean ignoreCRCRemerge = GlobalContext.getConfig().getMainProperties().getProperty("ignore.crcremerge", "true").equals("true");
			return new AsyncResponseChecksum(ac.getIndex(), getSlaveObject().checkSum(ac.getArgs()));
			//return new AsyncResponseChecksum(ac.getIndex(), getSlaveObject().remergeCheckSum(ac.getArgs()));
		} catch (IOException e) {
			return new AsyncResponseException(ac.getIndex(), e);
		}
	}
