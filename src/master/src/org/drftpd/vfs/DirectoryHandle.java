...
public class DirectoryHandle extends InodeHandle implements
	...
	private void createRemergedFile(LightRemoteInode lrf, RemoteSlave rslave,
			boolean collision, String message) throws FileExistsException, FileNotFoundException {
		...
		if (newFile.getInodeCheckSum() == 0L && CRCRemerge()) {
				rslave.addNewFileToCrcCheckList(newFile);
		}
	}
	
	public void collisionHandler(LightRemoteInode lrf, RemoteSlave rslave, String message) {
	...
	public void remerge(List<LightRemoteInode> files, RemoteSlave rslave, long lastModified)
		...
		try {
			...
			updateCreationTime(lastModified);
		} catch (FileNotFoundException e) {
			...
			*/
			...
			if (destination == null) {
				...
				while (source != null) {
					if (source.isFile()) {
						try {
							createRemergedFile(source, rslave, false, message);
							...
							try {
								...
								getFileUnchecked(source.getName()).updateCreationTime(source.lastModified());
								updateCreationTime(source.lastModified());
							} catch (ObjectNotValidException e1) {
								// File has collided with a dir/link in VFS, create this
								// as a collision
								if (collisionhandle()) {
									collisionHandler(source, rslave, message);
								} else {
									try {
										createRemergedFile(source, rslave, true, message);
			...
			if (compare < 0) {
				...
				if (destination.isLink()) {
					...
					// name collision
					if (source.isFile()) {
						message = "In remerging " + rslave.getName()
					...
					long sourceCRC = 1L;
					long destinationCRC = 1L;
					if (CRCRemerge()) {
						try {
							sourceCRC = rslave.getCheckSumForPath(getPath()
					...
					if (source.length() != destinationFile.getSize()
						...
						if (rslaves.contains(rslave) && rslaves.size() == 1) {
							...
							destinationFile.updateCreationTime(source.lastModified());
							updateCreationTime(source.lastModified());
						} else {
					...
					destination.updateCreationTime(source.lastModified());
				} else {
