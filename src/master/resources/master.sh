...
# Check for 0byte files in critical files
CHECK_FILE="0"
...
if [ "$CHECK_FILE" -eq "1" ];then
	backup_dir=/home/bot/backup/
	live_dir=/home/bot/master/

	backup_conf=$backup_dir/conf
	backup_slaves=$backup_dir/slaves
	backup_users=$backup_dir/javabeans

	live_conf=$live_dir/conf
	live_slaves=$live_dir/slaves
	live_users=$live_dir/users/javabeans

#	for i in `find $live_conf -type f \( ! -iname "*.tmp" \) -size 0`; do 
#		find $backupdir -maxdepth 1 -type f -name '*simple*' | sort -n | tar -vxf `tail -1` \
#			if [ "`du -sk $backup_conf/`basename $i` | awk '{print $1}'`" -gt "2" ] ; then
#				cp $backup_conf/`basename $i` live_conf/`dirname $i`
#			else
#				find $backupdir -maxdepth 1 -type f -name '*simple*' | sort -n | tar -vxf `head -n 1` \
#				cp $backup_conf/`basename $i` live_conf/`dirname $i`
#			fi
#	; done
#	
#
#	for i in `find $live_slaves -type f \( ! -iname "*.tmp" \) -size 0`; do 
#		find $backupdir -maxdepth 1 -type f -name '*simple*' | sort -n | tar -vxf `tail -1` \
#			if [ "`du -sk $backup_conf/`basename $i` | awk '{print $1}'`" -gt "2" ] ; then
#				cp $backup_conf/`basename $i` live_slaves/`dirname $i`
#			else
#				find $backupdir -maxdepth 1 -type f -name '*simple*' | sort -n | tar -vxf `head -n 1` \
#				cp $backup_conf/`basename $i` live_slaves/`dirname $i`
#			fi
#	; done
#	
#	for i in `find $live_users -type f \( ! -iname "*.tmp" \) -size 0`; do 
#		find $backupdir -maxdepth 1 -type f -name '*simple*' | sort -n | tar -vxf `tail -1` \
#			if [ "`du -sk $backup_conf/`basename $i` | awk '{print $1}'`" -gt "2" ] ; then
#				cp $backup_conf/`basename $i` live_users/`dirname $i`
#			else
#				find $backupdir -maxdepth 1 -type f -name '*simple*' | sort -n | tar -vxf `head -n 1` \
#				cp $backup_conf/`basename $i` live_users/`dirname $i`
#			fi
